...
// Área do entregador (/entregador). Estes wrappers existiam como funções
// locais e se perderam na modularização — sem eles o React quebrava com
// "DriverApp is not defined" e a tela ficava preta.
function DriverApp({ store, now }) {
  return (
    <ErrorBoundary>
      <React.Suspense fallback={<div style={{ padding: 20, textAlign: 'center', color: '#888' }}>Carregando área do entregador...</div>}>
        <DriverAppModular store={store} now={now} />
      </React.Suspense>
    </ErrorBoundary>
  );
}
...
function TVPanelApp({ store, now }) {
  return (
    <ErrorBoundary>
      <React.Suspense fallback={<div style={{ padding: 20, textAlign: 'center', color: '#888' }}>Carregando Painel TV...</div>}>
        <TVPanelAppModular store={store} now={now} />
      </React.Suspense>
    </ErrorBoundary>
  );
}
...
export default function App() {
  ...
  const store = {
    ...
    assignDriver: (id, driverId) => {
      // resolve em true/false (nunca rejeita): os painéis chamam sem catch,
      // e a área do entregador precisa saber se o aceite funcionou de verdade.
      return api(`/api/orders/${id}/driver`, { method: "PATCH", body: { driverId } })
        .then(() => {
